...
import type { AdCleanupPreviewResponse } from '../../../../services/ocean-engine.service'
import { CleanupStepSection } from '../shared/CleanupStepSection'

interface AdCleanupPreviewPanelProps {
  previewData: AdCleanupPreviewResponse['data'] | null
...
export const AdCleanupPreviewPanel: React.FC<AdCleanupPreviewPanelProps> = ({
  previewData,
  isPreviewStale,
  isPreviewing,
  isDeleting,
  ...
  return (
    <CleanupStepSection
      step={3}
      title="预览与执行"
      description="先预览待清理广告并确认数量，再提交删除任务。"
      className={
        hasPreview && !isPreviewStale && eligibleCount > 0 ? 'border-destructive/30' : undefined
      }
    >
      <div className="space-y-4">
        {isPreviewing && (
          <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>正在扫描广告，详细进度请查看底部日志面板…</span>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3 rounded-lg border bg-background p-3 sm:grid-cols-3">
              <div className="text-center sm:text-left">
                <div className="text-xs text-muted-foreground">扫描广告</div>
              ...
              <div className="text-center sm:text-left">
                ...
                <div className="text-lg font-semibold">
                  前 {Math.min(previewData.ads.length, 20)} 条
                </div>
              </div>
            </div>

            {previewData.ads.length > 0 ? (
              <div className="max-h-48 overflow-auto rounded-md border bg-background">
                {previewData.ads.slice(0, 20).map((ad) => (
                  <div
                    key={`${ad.advertiser_id}-${ad.promotion_id}`}
                    className="border-b px-3 py-2 text-sm last:border-b-0"
                  >
                    <div className="truncate font-medium">
                      {ad.promotion_name || ad.promotion_id}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      账户 {ad.advertiser_id} · 广告 {ad.promotion_id} · 创建时间{' '}
                      {ad.promotion_create_time}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {!hasPreview && !isPreviewing && (
          <div className="rounded-lg border border-dashed px-3 py-4 text-center text-sm text-muted-foreground">
            配置完成后点击「预览待清理广告」，系统将扫描并展示符合条件的广告。
          </div>
        )}
            ...
            <>
              ...
              {hasPreview && (
                ...
                >
                  ...
                  {eligibleCount > 0 ? `确认删除 ${eligibleCount} 个广告` : '提交广告清理任务'}
                </Button>
              )}
            </>
          )}
