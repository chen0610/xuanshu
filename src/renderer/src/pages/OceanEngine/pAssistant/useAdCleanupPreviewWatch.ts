...
const PREVIEW_JOB_ID_KEY = 'p-assistant-ad-cleanup-preview-job-id'
const PREVIEW_DATA_KEY = 'p-assistant-ad-cleanup-preview-data'
...
export function readPersistedPreviewData(): AdCleanupPreviewResponse['data'] | null {
  try {
...
export function persistPreviewData(data: AdCleanupPreviewResponse['data'] | null): void {
  try {
    if (data == null) {
...
interface UseAdCleanupPreviewWatchOptions {
  ...
  setPreviewData: (data: AdCleanupPreviewResponse['data'] | null) => void
...
export function useAdCleanupPreviewWatch({
  addLog,
  setLoading,
  setError,
  ...
  setPreviewData,
  setIsPreviewing
}: UseAdCleanupPreviewWatchOptions) {
  ...
  const applyPreviewResult = useCallback(
    (jobId: number, resultData: AdCleanupPreviewResponse['data']) => {
  ...
  const extractPreviewResultData = useCallback(
    (job: { result?: Record<string, unknown> | null }): AdCleanupPreviewResponse['data'] | null => {
      const raw = job.result
      if (!raw || typeof raw !== 'object') return null
      if (raw.data && typeof raw.data === 'object') {
        return raw.data as AdCleanupPreviewResponse['data']
      }
      if (typeof raw.total === 'number' && typeof raw.eligible_count === 'number') {
        return raw as AdCleanupPreviewResponse['data']
      }
      return null
    },
    []
  ...
  const syncPreviewJobOnce = useCallback(
    async (
      ...
      if (job.status === 'success') {
        ...
        if (options.resume) {
          addLog(
            `预览已完成：共扫描 ${resultData.total} 个广告，符合清理条件 ${resultData.eligible_count} 个`,
            'success'
          )
        }
        return 'done'
  ...
  const watchPreviewJob = useCallback(
    async (jobId: number, options: WatchPreviewOptions = {}) => {
      ...
      try {
        while (previewPollingRef.current && previewWatchTokenRef.current === token) {
          ...
          try {
            ...
            if (job.status === 'success') {
              ...
              applyPreviewResult(jobId, resultData)
              addLog(
                `预览完成：共扫描 ${resultData.total} 个广告，符合清理条件 ${resultData.eligible_count} 个`,
                'success'
              )
              break
            }
        ...
        const errorMessage = extractPreviewErrorMessage(err, '广告清理预览失败')
  ...
  useEffect(() => {
    ...
    // 切 Tab 卸载时可能遗留 loading / isPreviewing，先重置再根据 Job 状态恢复
    setLoading(false)
    setIsPreviewing(false)
    ...
    // Tab 重新挂载时恢复预览任务；允许 StrictMode 二次执行
