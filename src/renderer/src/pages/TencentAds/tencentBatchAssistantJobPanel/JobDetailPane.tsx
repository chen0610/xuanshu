...
import type {
  TencentBatchAssistantJobDetail,
  TencentBatchAssistantJobEvent
} from '../../../services/tencent-ads.service'
import { AccountResultsSection } from './AccountResultsSection'
import {
  canRetryJob,
...
interface JobDetailPaneProps {
  job: TencentBatchAssistantJobDetail | null
  events: TencentBatchAssistantJobEvent[]
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5">
                  已执行 {getJobRunningMinutes(job)} 分钟，可能因部署重启或 Worker 中断仍停留在执行中。
                </p>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {getTencentJobTypeLabel(job.job_type)} · {formatJobTime(job.created_at)}
          </p>
          {job.parent_job_id ? (
        ...
        <AccountResultsSection accountResults={accountResults} />
...
const EventTimelineItem: React.FC<{
  event: TencentBatchAssistantJobEvent
  isLast: boolean
}> = ({ event, isLast }) => {
