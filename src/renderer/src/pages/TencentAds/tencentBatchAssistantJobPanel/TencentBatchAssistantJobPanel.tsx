...
import {
  ...
  type TencentBatchAssistantJobDetail,
  type TencentBatchAssistantJobEvent,
  type TencentBatchAssistantJobItem
...
import {
  ...
  type TencentJobListMeta,
  type TencentRetryMode
} from './jobUi'

export const TencentBatchAssistantJobPanel: React.FC<{
  refreshToken: number
  focusJobId: number | null
  onClose?: () => void
  onActiveCountChange?: (count: number) => void
}> = ({ refreshToken, focusJobId, onClose, onActiveCountChange }) => {
  const [jobs, setJobs] = useState<TencentBatchAssistantJobItem[]>([])
  const [selectedJob, setSelectedJob] = useState<TencentBatchAssistantJobDetail | null>(null)
  const [events, setEvents] = useState<TencentBatchAssistantJobEvent[]>([])
  ...
  const [pageMeta, setPageMeta] = useState<TencentJobListMeta>({
    total: 0,
    page: 1,
    page_size: PAGE_SIZE,
    total_pages: 0,
  ...
  const [retryingJob, setRetryingJob] = useState<{ id: number; mode: TencentRetryMode } | null>(
    null
  )
  ...
  const loadJobDetail = useCallback(async (jobId: number): Promise<void> => {
    ...
    try {
      const [job, eventResponse] = await Promise.all([
        tencentBatchAssistantJobService.getJob(jobId),
        tencentBatchAssistantJobService.listJobEvents(jobId, { limit: 120 })
      ])
      setSelectedJob(job)
      setEvents([...eventResponse.items].reverse())
      setError('')
    } catch (err) {
      console.error('Failed to load tencent batch assistant job detail:', err)
  ...
  const loadJobs = useCallback(
    async ({
      ...
      try {
        ...
        const response = await tencentBatchAssistantJobService.listJobs({
          page: targetPage,
          page_size: PAGE_SIZE,
        ...
        console.error('Failed to load tencent batch assistant jobs:', err)
  ...
  const handlePauseJob = async (job: TencentBatchAssistantJobItem): Promise<void> => {
    const confirmed = window.confirm(`确认暂停任务 #${job.id}？暂停后可重新运行。`)
    if (!confirmed) return

    setPausingJobId(job.id)
    try {
      const response = await tencentBatchAssistantJobService.cancelJob(job.id)
      if (response.code !== 0) {
        throw new Error(response.msg || '暂停任务失败')
      }
      await loadJobs({ preferredJobId: job.id })
    } catch (err) {
      setError(err instanceof Error ? err.message : '暂停任务失败，请稍后重试')
    } finally {
      setPausingJobId(null)
    }
  }

  const handleResumeJob = async (job: TencentBatchAssistantJobItem): Promise<void> => {
    ...
    try {
      const response = await tencentBatchAssistantJobService.resumeJob(job.id)
      if (response.code !== 0) {
        throw new Error(response.msg || '重新运行任务失败')
      }
      await loadJobs({ preferredJobId: job.id })
    } catch (err) {
      setError(err instanceof Error ? err.message : '重新运行任务失败，请稍后重试')
    } finally {
      setResumingJobId(null)
    }
  }

  const handleRecoverJob = async (job: TencentBatchAssistantJobItem): Promise<void> => {
    ...
    try {
      const response = await tencentBatchAssistantJobService.recoverJob(job.id)
      if (response.code !== 0) {
        throw new Error(response.msg || '人工恢复任务失败')
      }
      await loadJobs({ preferredJobId: job.id })
    } catch (err) {
      setError(err instanceof Error ? err.message : '人工恢复任务失败，请稍后重试')
    } finally {
  ...
  const handleRetryJob = async (
    ...
    try {
      const response = await tencentBatchAssistantJobService.retryJob(job.id, { mode })
      if (response.code !== 0) {
        throw new Error(response.msg || '创建重试任务失败')
      }
      setPage(1)
      await loadJobs({ targetPage: 1, preferredJobId: response.job_id })
    } catch (err) {
      setError(err instanceof Error ? err.message : '创建重试任务失败，请稍后重试')
    } finally {
      setRetryingJob(null)
    }
  }
