...
import { Loader2, CheckCircle, XCircle, Tag, X } from 'lucide-react'
import {
  Card,
  CardHeader,
  CardTitle,
...
import { accountRemarkService } from '../../services/tencent-ads.service'
...
export const AccountRemarkPage: React.FC = () => {
  const [configs, setConfigs] = useState<Config[]>([])
  const [loading, setLoading] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [selectedConfigId, setSelectedConfigId] = useState<number | null>(null)
  const [accountIds, setAccountIds] = useState<string>('')
  const [operationType, setOperationType] = useState<'remark' | 'tag'>('remark')
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    // 验证标签操作的组织ID
    if (operationType === 'tag' && !orgId.trim()) {
    ...
    try {
      const operationTypeText = operationType === 'remark' ? '修改备注' : '更新标签'
      addLog(
        `开始批量${operationTypeText} ${accountIdList.length} 个账号${accountIdList.length > 1 ? `（起始数字: ${startNumber}）` : ''}...`,
        'info'
      )
      ...
      const payload = {
        ...
        org_id: operationType === 'tag' ? orgId.trim() : undefined
      }

      addLog('正在并发处理所有账号，请稍候...', 'info')
      const response = await accountRemarkService.batchModifyAccountRemark(payload)
      ...
      if (response.data) {
        ...
        // 显示详细结果
        account_results.forEach((result: any) => {
          if (result.success) {
            addLog(`✅ 账号 ${result.account_id} ${operationTypeText}成功`, 'success')
          } else {
            addLog(
              `❌ 账号 ${result.account_id} ${operationTypeText}失败: ${result.error || '未知错误'}`,
              'error'
            )
          }
        })
        ...
        // 最终统计
        addLog(
          `批量${operationTypeText}完成! 成功: ${total_success}, 失败: ${total_error}`,
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>批量修改账号备注或标签，支持多个账号自动编号</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            ...
            <div className="flex gap-6 items-center">
              <div className="flex items-center gap-2">
                <input
                  ...
                  name="operation-type"
                  value="remark"
                  checked={operationType === 'remark'}
                  onChange={() => setOperationType('remark')}
                  disabled={isSubmitting}
                  className="w-4 h-4"
                />
                <Label htmlFor="operation-remark" className="cursor-pointer font-normal">
                  备注
                </Label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  ...
                  name="operation-type"
                  value="tag"
                  checked={operationType === 'tag'}
                  onChange={() => setOperationType('tag')}
                  disabled={isSubmitting}
                  className="w-4 h-4"
                />
                <Label htmlFor="operation-tag" className="cursor-pointer font-normal">
                  标签
                </Label>
              </div>
            </div>
          ...
          {operationType === 'tag' && (
            <div className="grid gap-2">
              ...
              <Input
                ...
                className="h-11"
              />
              <p className="text-sm text-muted-foreground">标签操作时必须填写组织ID</p>
            </div>
          )}
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              className="h-11"
            />
            <p className="text-sm text-muted-foreground">多个账号时将自动编号，可设置起始数字</p>
          </div>
          ...
          <div className="grid gap-2">
            ...
            <div className="flex gap-4 items-center">
              ...
              <span className="text-sm text-muted-foreground flex-2">从几开始编号</span>
            </div>
          </div>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  {operationType === 'remark' ? '修改中...' : '更新中...'}
                </>
              ) : (
                <>
                  <Tag className="mr-2 w-4 h-4" />
                  {operationType === 'remark' ? '批量修改' : '批量更新标签'}
                </>
              )}
            </Button>
          </div>
